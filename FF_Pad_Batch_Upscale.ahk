...
FF_Audios := ["-c:a copy -map 0?", "-an", "-c:a copy -map 0:a:0", "-c:a copy -map 0:a:2", "-filter_complex amerge=inputs=2", "-map 0:a?"]
FF_CRF := 34
FF_Custom := ""
FF_Command := ""
...
SetWorkingDir, D:\Videos\


Loop, Files, *.mp4
{
	FF_FileIn := A_LoopFileName
	msgbox, %FF_FileIn%
}
...
If !(FF_FileIn ~= "youtube.com") { ; Refactor this function later so only necessary code is run per situation (file or URL).
	...
	If (Type = "bat") { ; Put these two, plus clipboard, into new function FF_Export() (maybe)
		...
		FF_Command := " -hide_banner " . (FF_Upscale ? "" : FF_trimSS) . (FF_Upscale ? "-r " outFPS : "") . " -i """ . fileDir . "\" . fileName . """ " . FF_Codecs . " " . FF_Custom . " " . FF_Codec_Extras_V . FF_Resolution . " -crf " . FF_CRF . " " . FF_Audio . (FF_Upscale && FF_trimTo != "-ss" ? "" : FF_trimTo) . " """ . FF_FileOut . """ " . FF_Overwrite
	...
	Tooltip, % FileName . "`n" . FF_Codecs . " " . FF_Audio . FF_trimSS . FF_trimTo . "`n(-crf " . FF_CRF . " " . FF_Custom . " " FF_Overwrite . FF_FileExist ")`nMode: " . FF_trimMode . "|" . Display_RIFE . "FPS: " . inFPS . "|" . fileExt . "|" . Display_Save . Display_Upscale . "`n`n " . FF_Queue ; FF_Interp_Mode ; . " " . FF_Upscale
	;Chunky := 
	}
}
...
; Set Working Directory to wherever the file is for cleaner code, resetting after.
...
; Implement remembering and restoring last command?
; Implement Chunky
; Implement interpolation and upscaling
; Implement delegation to Ally (Unnecessary, SyncThing and .bat export serve this function and more.)
